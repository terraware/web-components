...
env:
  ...
  BUILDKITE_PLUGIN_S3_CACHE_PREFIX: 'repo/web-components'
  ...
  - label: ':pipeline: Upload pipeline'
    plugins:
      - peakon/git-shallow-clone#v0.0.1:
          ...
          depth: 200
    command: 'if [ -f .buildkite/pipeline.yml ]; then buildkite-agent pipeline upload; fi'
```
...
Artifacts are per-job; we use them to pass data between steps. The `dist` directory gets bundled
up in a compressed tarfile so it can be downloaded by later steps.

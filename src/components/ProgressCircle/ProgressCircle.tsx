...
export default function ProgressCircle(props: Props): JSX.Element {
  ...
  return (
    <Box className='circleContainer'>
      ...
      <Box className='labelContainer'>{value && <p className={`progressCircle_label__${size}`}>{`${Math.round(value)}%`}</p>}</Box>
      <CircularProgress value={value} variant={determinate ? 'determinate' : 'indeterminate'} className={`${size} circleFill`} />
    </Box>
  );
}
